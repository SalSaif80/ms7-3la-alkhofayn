...
let ms7Array;
if(localStorage.ms7ArrayStorage != null){
    ms7Array = JSON.parse(localStorage.ms7ArrayStorage)
}else{
    ms7Array = []
}
...
function set_locale_storage(){
    ...
    localStorage.setItem("ms7ArrayStorage", toStringArray)
}
...
calcBtn.addEventListener('click', function(event){
    ...
    if(!(end_msh_time() === undefined) && !(radio_button_checked() === undefined) && !(minute.value === 'الدقائق') && !(hour.value === 'الساعة') ){
        document.getElementById('endMshTime').innerHTML = `
            تنتهي مدة المسح في يوم ${end_msh_time()}، في تمام الساعة ${document.getElementById('hour').value}:${document.getElementById('minute').value} ${selectedSize}
        `
    }else{
